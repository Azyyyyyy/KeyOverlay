...
namespace KeyOverlay
{
    public class PerformanceGraph
    {
        ...
        private const uint GraphHistoryCount = 100;
        ...
        private readonly float[] _values = new float[GraphHistoryCount];

        private uint _head;

        public float GraphAverage => _values.Sum() / GraphHistoryCount;
        ...
        public void Render(float x, float y, Nvg nvg)
        {
            ...
            {
                for (uint i = 0; i < GraphHistoryCount; i++)
                {
                    float v = 1.0f / (0.00001f + _values[(_head + i) % GraphHistoryCount]);
                    v = MathF.Min(v, 80.0f);
                    float vx = ((float)i / (GraphHistoryCount - 1)) * w;
            ...
            else if (_style == GraphRenderStyle.Percent)
            {
                for (uint i = 0; i < GraphHistoryCount; i++)
                {
                    float v = _values[(_head + i) % GraphHistoryCount] * 1.0f;
                    v = MathF.Min(v, 100.0f);
                    float vx = x + ((float)i / (GraphHistoryCount - 1)) * w;
            ...
            else if (_style == GraphRenderStyle.Ms)
            {
                for (uint i = 0; i < GraphHistoryCount; i++)
                {
                    float v = _values[(_head + i) % GraphHistoryCount] * 1000.0f;
                    v = MathF.Min(v, 20.0f);
                    float vx = x + ((float)i / (GraphHistoryCount - 1)) * w;
